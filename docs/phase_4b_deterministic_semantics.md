...
*Note.* This embedded explanatory note canonicalizes the framework property "T8 Authority Singularity" per `docs/phase_4b_step11_closure_verification.md` §4 — the substrate's authority topology articulated as a function `authority : Concern → Authority` with `|authority(c)| = 1` for every concern `c`; aggregate consequence of D-FAULT-2 (single-emitter) + D-SESS-1 (sole mutator) + D-SCHED-1 + D-SCHED-12 (pure-function discipline) per closure-verification §4.2. Unlike Framework Theorems T1, T4, T5 — each of which appears as a numbered Theorem in `docs/phase_4b_step11_admissibility_framework.md` — T8 is articulated in the closure-verification analysis as a promotion candidate rather than a numbered admissibility-framework Theorem; per codification plan §1 row 7 ("T8 Authority Singularity | C-2 embedded | §5 D-SESS"), this embedded note IS the canonical contract statement of T8 (parallel to Wave 5 AAU 5.4 Drain Epoch, which canonicalizes a framework-derived primitive at the glossary level rather than via a numbered framework Theorem). T8 is classified **NORMATIVE-CANDIDATE** per closure-verification §4.3 (promotion-recommended); the codification plan §1 elected C-2 embedded form to control contract-surface inflation per its §1 line 27 ("Embedding T1/T4/T5/T8 saves 4 clauses of contract-surface inflation while preserving all citation needs"). The embedded form codifies T8's reasoning without introducing a new clause; authority singularity is structurally already enforced by the four anchor clauses above. Future ingress-kind or authority-shape proposals are testable against this singularity as a single citable proposition rather than by re-citing D-FAULT-2 + D-SESS-1 + D-SCHED-1/-12 separately. No new authority surface, no replay-identity widening, no ingress widening, no scheduler widening, no session-mutation widening, no `orchestration_tick`-supremacy widening, no transport-discipline widening. V9 framework-label confinement preserved (the framework label "T8" appears only in this Note section and in the subsection heading).
